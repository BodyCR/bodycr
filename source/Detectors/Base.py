...
class Capture:
    ...
    def __init__(self, pose=Prefabs.POSE.normal, hands=Prefabs.HANDS.normal, face=Prefabs.FACE.normal, all=Prefabs.ALL.normal, gpu=Prefabs.GPU_ACELERATION) -> None:
        """
            ...
            ##### `Capture.__init__(pose: PoseConfiguration, hands: HandsConfiguration, face: FaceConfiguration, holistic: HolisticConfiguration, gpu: GPUAceleration)`
    ...
    def Read(self, img, indentifier=Prefabs.DETECT_ALL) -> None:
        """
            Update all features: Pose and Hands

            ### Parameters:
                `img: Mat` - The OpenCV Image
                `identifier: DetectionIdentifier = Prefabs.DETECT_HOLISTIC` - If True, the pose will be recognized
        """

        pose, hand, face = (int(i)==1 for i in indentifier)

        if pose == True and hand == True and face == True:
            self.__holistic.Read(img)
            self.pose = self.__holistic.pose
            self.hands = [self.__holistic.hands["left"], self.__holistic.hands["right"]]
            self.face = self.__holistic.face
            return
